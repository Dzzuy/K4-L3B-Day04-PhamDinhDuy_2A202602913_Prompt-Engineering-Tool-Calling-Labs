...
export default function ChatAgentView() {
    ...
    fetch("/api/datasets/available")
      ...
      .then((data: any[]) => {
        if (Array.isArray(data) && data.length > 0) {
          ...
          setActiveFileId((current) => current || mapped[0].id);
        }
      })
      .catch(() => {});
    ...
    fetch("/api/conversations")
      ...
      .then((data: any[]) => {
        if (Array.isArray(data) && data.length > 0) {
          ...
          setActiveFileId(data[0].file_id);
        }
      })
      .catch(() => {});
  }, []);
  ...
  const handleNewChat = async () => {
    if (!activeFile.id) {
      setRequestError("Chọn hoặc tải lên một CSV trước khi tạo đoạn chat.");
      return;
    }
    ...
    try {
      ...
      if (res.ok) {
        ...
        setActiveFileId(created.file_id);
        return;
      }
    } catch {
      // A visible error is shown below; no local conversation is fabricated.
    }
    setRequestError("Không thể tạo đoạn chat với backend. Kiểm tra PrivacyGuard API rồi thử lại.");
  };

  const handleSelectFile = async (fileItem: CsvFileItem) => {
    setActiveFileId(fileItem.id);
    try {
      const response = await fetch("/api/conversations", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ file_id: fileItem.id, title: `Phân tích ${fileItem.name}` }),
      });
      ...
      setRequestError("Không thể chuyển dataset trong phiên chat. Kiểm tra backend rồi thử lại.");
    }
  };
  ...
  const handleFileUpload = async (e: ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        newFileItem = {
          id: data.dataset_id || data.file_id,
          name: data.filename,
          records: data.row_count,
          cols: data.col_count || data.column_count,
      ...
      setLoading(false);
      setRequestError("Tải CSV lên backend không thành công. Không tạo dataset hoặc trace giả.");
      return;
    }
    ...
    await handleSelectFile(newFileItem);
  };

  const handleSendMessage = async (textToSend?: string) => {
    const query = (textToSend || inputText).trim();
    if (!query || !activeConvId || !activeFile.id || !activeConv) {
      setRequestError("Chọn một dataset và đoạn chat trước khi gửi yêu cầu.");
      return;
    }
    ...
    try {
      ...
      // The UI never manufactures a tool result if the real request fails.
    }
    setLoading(false);
    setRequestError("Không thể nhận phản hồi từ PrivacyGuard API. Không có tool trace nào được tạo.");
    return;

    /* Legacy local demo fallback intentionally disabled: all displayed traces must come from /api/chat.
    setTimeout(() => {
      ...
      <div className="flex-1 flex flex-col h-full bg-white relative overflow-hidden">
        ...
        <header className="h-14 border-b border-zinc-200 px-5 flex items-center justify-between shrink-0 bg-white/80 backdrop-blur-md z-10">
          <div className="flex items-center gap-3">
            ...
            <div className="flex items-center gap-2 px-3 py-1.5 bg-zinc-100/80 border border-zinc-200 rounded-full text-xs font-medium text-zinc-800">
              ...
              <span>PrivacyGuard Agent · v3 · OpenRouter · openai/gpt-4.1-mini</span>
            </div>
        ...
        <div className="flex-1 overflow-y-auto px-4 py-8">
          <div className="max-w-3xl mx-auto space-y-7">
                            ...
                            >
                              ...
                              >
                                <div className="flex items-center gap-2.5">
                                  ...
                                  <span className={`text-[11px] font-medium flex items-center gap-1 ${t.status === "success" ? "text-emerald-700" : "text-amber-700"}`}>
                                    <span className={`w-1.5 h-1.5 rounded-full ${t.status === "success" ? "bg-emerald-500" : "bg-amber-500"}`} />
                                    {t.status}
                                  </span>
                                </div>
                              ...
                              {isOpen && (
                                <div className="px-4 py-3 bg-white border-t border-zinc-200/80 text-xs font-mono space-y-2">
                                  <p className="text-[10px] font-semibold uppercase tracking-wider text-zinc-400">Tool input</p>
                                  <pre className="max-h-36 overflow-auto rounded-md bg-zinc-900 p-2 text-[10px] leading-4 text-zinc-100">{JSON.stringify(t.args || {}, null, 2)}</pre>
                                  <p className={`text-[10px] font-semibold uppercase tracking-wider ${t.status === "success" ? "text-zinc-400" : "text-amber-700"}`}>
                                    {t.status === "success" ? "Tool result" : "Tool error"}
                                  </p>
                                  <pre className="max-h-52 overflow-auto rounded-md bg-zinc-50 p-2 text-[10px] leading-4 text-zinc-700">{JSON.stringify(t.result, null, 2)}</pre>
                                </div>
                              )}
                            </div>
                          );
            ...
            {requestError && (
              <div className="max-w-xl rounded-xl border border-red-200 bg-red-50 p-3 text-xs text-red-800">
                {requestError}
              </div>
            )}
            <div ref={chatEndRef} />
